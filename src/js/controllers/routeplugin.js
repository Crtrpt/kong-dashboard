angular.module('app').controller("RoutePluginController", ["$scope", "Kong", "$location", "$routeParams","plugins", "apis","services","routes", "consumers", "plugin", "Alert", "$route", function ($scope, Kong, $location, $routeParams, plugins, apis, services,routes,consumers, plugin, Alert, $route)
{
    var mode;
    if (plugin) {
        $scope.title = "Edit Routes";
        $scope.action = "Save";
        mode = 'edit';
    } else {
        $scope.title = "Add Routes";
        $scope.action = "Add";
        mode = 'create';
    }
    ...
    $scope.save = function () {
        ...
        var endpoint = '/routes/'+$scope.plugin.routes_id+"/plugins";
        delete $scope.plugin['routes_id'];
        var data = $scope.plugin;
        delete data.routes_id
        console.log(data);
