angular.module('app').controller("ServicePluginController",
     ["$scope", "Kong", "$location", "$routeParams","plugins", "apis","services", "consumers", "plugin", "Alert", "$route",
      function ($scope, Kong, $location, $routeParams, plugins, apis, services,consumers, plugin, Alert, $route)
{
    var mode;
    if (plugin) {
    ...
    $scope.schema = {
        properties: {
            'service_id': {
                ...
                label: 'Which SERVICES(s) should this plugin apply to?'
            },
            'name': {
                required: true,
                type: 'string',
